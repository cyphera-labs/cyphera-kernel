...
const PROC_LEN: usize = include_bytes!(env!("PROC_CLOCK_SETTIME_ELF_PATH")).len();

#[repr(C, align(8))]
struct AlignedProc([u8; PROC_LEN]);

static PROC_ALIGNED: AlignedProc =
    AlignedProc(*include_bytes!(env!("PROC_CLOCK_SETTIME_ELF_PATH")));

const PROC_ELF: &[u8] = &PROC_ALIGNED.0;

const STACK_VADDR: u64 = 0x7028_0000;
...
pub extern "C" fn kernel_main(boot_info_ptr: u32) -> ! {
    uart::init();
    println!(
        "[test] clock_settime: bringing up frame; proc_clock_settime={} bytes",
        PROC_ELF.len()
    );
    ...
    let loaded = kernel::elf::load_static(PROC_ELF, &mut vmspace).expect("load proc_clock_settime");
    let _ = vmspace
        .map_anon(
    ...
    println!(
        "[test] clock_settime: registered pid {} (entry {:#x})",
        pid.raw(),
        loaded.entry
    );
    println!("------ user output ------");
