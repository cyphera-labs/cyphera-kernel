...
const PROC_LEN: usize = include_bytes!(env!("PROC_PTRACE_TRACEFORK_ELF_PATH")).len();

#[repr(C, align(8))]
struct AlignedProc([u8; PROC_LEN]);

static PROC_ALIGNED: AlignedProc =
    AlignedProc(*include_bytes!(env!("PROC_PTRACE_TRACEFORK_ELF_PATH")));
...
pub extern "C" fn kernel_main(boot_info_ptr: u32) -> ! {
    uart::init();
    println!(
        "[test] ptrace_tracefork: bringing up frame; proc_ptrace_tracefork={} bytes",
        PROC_ELF.len()
    );
    ...
    let loaded =
        kernel::elf::load_static(PROC_ELF, &mut vmspace).expect("load proc_ptrace_tracefork");
    let _ = vmspace
        .map_anon(
    ...
    println!(
        "[test] ptrace_tracefork: registered pid {} (entry {:#x})",
        pid.raw(),
        loaded.entry
    );
    println!("------ user output ------");
