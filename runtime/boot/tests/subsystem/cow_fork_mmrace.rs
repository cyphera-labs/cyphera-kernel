...
const PROC_LEN: usize = include_bytes!(env!("PROC_COW_FORK_MMRACE_ELF_PATH")).len();

#[repr(C, align(8))]
struct AlignedProc([u8; PROC_LEN]);

static PROC_ALIGNED: AlignedProc =
    AlignedProc(*include_bytes!(env!("PROC_COW_FORK_MMRACE_ELF_PATH")));
...
pub extern "C" fn kernel_main(boot_info_ptr: u32) -> ! {
    uart::init();
    println!(
        "[test] cow_fork_mmrace: bringing up frame; proc_cow_fork_mmrace={} bytes",
        PROC_ELF.len()
    );
    ...
    let mut vmspace = VmSpace::new_user().expect("alloc proc_cow_fork_mmrace vmspace");
    let loaded = kernel::loader::elf::load_static(PROC_ELF, &mut vmspace)
        .expect("load proc_cow_fork_mmrace");
    let _ = vmspace
        .map_anon(
        ...
        .expect("map proc_cow_fork_mmrace stack");
    ...
    println!(
        "[test] cow_fork_mmrace: registered pid {} (entry {:#x})",
        pid.raw(),
        loaded.entry
    );
    println!("------ user output ------");
