...
const PROC_LEN: usize = include_bytes!(env!("PROC_MISC_QUICKWINS_ELF_PATH")).len();

#[repr(C, align(8))]
struct AlignedProc([u8; PROC_LEN]);

static PROC_ALIGNED: AlignedProc =
    AlignedProc(*include_bytes!(env!("PROC_MISC_QUICKWINS_ELF_PATH")));

const PROC_ELF: &[u8] = &PROC_ALIGNED.0;

const STACK_VADDR: u64 = 0x7078_0000;
...
pub extern "C" fn kernel_main(boot_info_ptr: u32) -> ! {
    uart::init();
    println!(
        "[test] misc_quickwins: bringing up frame; proc_misc_quickwins={} bytes",
        PROC_ELF.len()
    );
    ...
    let loaded =
        kernel::loader::elf::load_static(PROC_ELF, &mut vmspace).expect("load proc_misc_quickwins");
    let _ = vmspace
        .map_anon(
    ...
    println!(
        "[test] misc_quickwins: registered pid {} (entry {:#x})",
        pid.raw(),
        loaded.entry
    );
    println!("------ user output ------");
