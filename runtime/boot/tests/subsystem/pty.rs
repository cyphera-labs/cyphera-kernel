...
const ELF_LEN: usize = include_bytes!(env!("PROC_PTY_ELF_PATH")).len();

#[repr(C, align(8))]
struct AlignedElf([u8; ELF_LEN]);

static ALIGNED: AlignedElf = AlignedElf(*include_bytes!(env!("PROC_PTY_ELF_PATH")));
const ELF: &[u8] = &ALIGNED.0;
...
pub extern "C" fn kernel_main(boot_info_ptr: u32) -> ! {
    uart::init();
    println!("[test] pty: bringing up frame");
    ...
    let mut vmspace = VmSpace::new_user().expect("alloc proc_pty vmspace");
    let loaded = kernel::loader::elf::load_static(ELF, &mut vmspace).expect("load proc_pty");
    let _ = vmspace
        .map_anon(
