...
const ELF_LEN: usize = include_bytes!(env!("PROC_OOM_ELF_PATH")).len();

#[repr(C, align(8))]
struct AlignedElf([u8; ELF_LEN]);

static ELF_ALIGNED: AlignedElf = AlignedElf(*include_bytes!(env!("PROC_OOM_ELF_PATH")));
...
pub extern "C" fn kernel_main(boot_info_ptr: u32) -> ! {
    uart::init();
    println!("[test] oom: bringing up frame");
    ...
    let mut vmspace = VmSpace::new_user().expect("alloc proc_oom vmspace");

    let loaded = kernel::loader::elf::load_static(ELF, &mut vmspace).expect("load proc_oom");
    let _ = vmspace
        .map_anon(
    ...
    println!("[test] oom: dropping to ring 3");
    println!("------ user output ------");
