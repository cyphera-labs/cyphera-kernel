...
const PROC_LEN: usize = include_bytes!(env!("PROC_PTRACE_SIGNAL_ELF_PATH")).len();

#[repr(C, align(8))]
struct AlignedProc([u8; PROC_LEN]);

static PROC_ALIGNED: AlignedProc =
    AlignedProc(*include_bytes!(env!("PROC_PTRACE_SIGNAL_ELF_PATH")));

const PROC_ELF: &[u8] = &PROC_ALIGNED.0;

const STACK_VADDR: u64 = 0x7068_0000;
...
pub extern "C" fn kernel_main(boot_info_ptr: u32) -> ! {
    uart::init();
    println!(
        "[test] ptrace_signal: bringing up frame; proc_ptrace_signal={} bytes",
        PROC_ELF.len()
    );
    ...
    let loaded = kernel::elf::load_static(PROC_ELF, &mut vmspace).expect("load proc_ptrace_signal");
    let _ = vmspace
        .map_anon(
    ...
    println!(
        "[test] ptrace_signal: registered pid {} (entry {:#x})",
        pid.raw(),
        loaded.entry
    );
    println!("------ user output ------");
