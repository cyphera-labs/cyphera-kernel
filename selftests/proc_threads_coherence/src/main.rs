...
pub extern "C" fn _start() -> ! {
    ...
    let coord = match sys_mmap(0, REGION_BYTES, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1i64 as u64, 0) {
        a if a < 0 => { log("coord mmap failed\n"); sys_exit(1); }
        a => a as u64,
    };
    ...
    if rc < 0 { log("clone failed\n"); sys_exit(1); }

    let r1 = sys_mmap(0, PAGE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1i64 as u64, 0);
    if r1 < 0 { fail(coord, 1); }
    unsafe { core::ptr::write_volatile(r1 as u64 as *mut u32, 0xA1A1) };
    cw(O_MMAP, r1 as u64);

    let r2 = sys_mmap(0, PAGE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1i64 as u64, 0);
    if r2 < 0 { fail(coord, 2); }
    cw(O_MUNMAP, r2 as u64);
    if sys_munmap(r2 as u64, PAGE) != 0 { fail(coord, 2); }

    let r3 = sys_mmap(0, PAGE, PROT_NONE, MAP_ANONYMOUS | MAP_PRIVATE, -1i64 as u64, 0);
    if r3 < 0 { fail(coord, 3); }
    if sys_mprotect(r3 as u64, PAGE, PROT_READ | PROT_WRITE) != 0 { fail(coord, 3); }
    unsafe { core::ptr::write_volatile(r3 as u64 as *mut u32, 0xA3A3) };
    cw(O_MPROT, r3 as u64);

    let r4 = sys_mmap(0, PAGE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1i64 as u64, 0);
    if r4 < 0 { fail(coord, 4); }
    unsafe { core::ptr::write_volatile(r4 as u64 as *mut u32, 0xA4A4) };
    let r4n = sys_mremap(r4 as u64, PAGE, 2 * PAGE, MREMAP_MAYMOVE, 0);
    if r4n < 0 { fail(coord, 4); }
    cw(O_MREMAP, r4n as u64);

    let r5 = sys_mmap(0, PAGE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1i64 as u64, 0);
    if r5 < 0 { fail(coord, 5); }
    unsafe { core::ptr::write_volatile(r5 as u64 as *mut u32, 0xA5A5) };
    if sys_madvise(r5 as u64, PAGE, MADV_DONTNEED) != 0 { fail(coord, 5); }
    cw(O_MADV, r5 as u64);

    let cur = sys_brk(0);
    if cur <= 0 { fail(coord, 6); }
    let newb = sys_brk(cur as u64 + PAGE);
    if newb < cur as u64 as i64 + PAGE as i64 { fail(coord, 6); }
    cw(O_BRK, cur as u64);

    let id7 = sys_shmget(IPC_PRIVATE, PAGE as usize, IPC_CREAT | 0o600);
    if id7 < 0 { fail(coord, 7); }
    let s7 = sys_shmat(id7 as i64, 0, 0);
    if s7 < 0 { sys_shmctl(id7 as i64, IPC_RMID, 0); fail(coord, 7); }
    ...
    if id8 < 0 { fail(coord, 8); }
    let s8 = sys_shmat(id8 as i64, 0, 0);
    if s8 < 0 { sys_shmctl(id8 as i64, IPC_RMID, 0); fail(coord, 8); }
    cw(O_SHMDT, s8 as u64);
    if sys_shmdt(s8 as u64) != 0 { fail(coord, 8); }
    ...
    if result != 0 {
        let mut buf = [0u8; 64];
        let n = format_kv(&mut buf, b"threads_coherence: peer did NOT observe op #", result);
...
fn fail(coord: u64, tag: i64) -> ! {
    ...
    peer_done.store(1, Ordering::Release);
    let mut buf = [0u8; 64];
    let n = format_kv(&mut buf, b"threads_coherence: leader mutation failed op #", tag);
    sys_write(1, buf.as_ptr(), n);
    sys_exit(1);
}
...
fn format_kv(buf: &mut [u8], prefix: &[u8], n: i64) -> usize {
    ...
    let mut d = 0;
    let mut v = if n < 0 { buf[i] = b'-'; i += 1; (-n) as u64 } else { n as u64 };
    if v == 0 { digits[0] = b'0'; d = 1; } else { while v > 0 { digits[d] = b'0' + (v % 10) as u8; v /= 10; d += 1; } }
...
fn sys_write(fd: u64, buf: *const u8, len: usize) -> i64 {
    let r: i64;
    unsafe { asm!("syscall", in("rax") 1u64, in("rdi") fd, in("rsi") buf, in("rdx") len, lateout("rax") r, out("rcx") _, out("r11") _, options(nostack)); }
    r
}
#[inline(never)]
fn sys_mmap(addr: u64, len: u64, prot: u64, flags: u64, fd: u64, off: u64) -> i64 {
    let r: i64;
    unsafe { asm!("syscall", in("rax") 9u64, in("rdi") addr, in("rsi") len, in("rdx") prot, in("r10") flags, in("r8") fd, in("r9") off, lateout("rax") r, out("rcx") _, out("r11") _, options(nostack)); }
    r
}
#[inline(never)]
fn sys_munmap(addr: u64, len: u64) -> i64 {
    let r: i64;
    unsafe { asm!("syscall", in("rax") 11u64, in("rdi") addr, in("rsi") len, lateout("rax") r, out("rcx") _, out("r11") _, options(nostack)); }
    r
}
#[inline(never)]
fn sys_mprotect(addr: u64, len: u64, prot: u64) -> i64 {
    let r: i64;
    unsafe { asm!("syscall", in("rax") 10u64, in("rdi") addr, in("rsi") len, in("rdx") prot, lateout("rax") r, out("rcx") _, out("r11") _, options(nostack)); }
    r
}
#[inline(never)]
fn sys_mremap(addr: u64, old: u64, new: u64, flags: u64, naddr: u64) -> i64 {
    let r: i64;
    unsafe { asm!("syscall", in("rax") 25u64, in("rdi") addr, in("rsi") old, in("rdx") new, in("r10") flags, in("r8") naddr, lateout("rax") r, out("rcx") _, out("r11") _, options(nostack)); }
    r
}
#[inline(never)]
fn sys_madvise(addr: u64, len: u64, advice: u64) -> i64 {
    let r: i64;
    unsafe { asm!("syscall", in("rax") 28u64, in("rdi") addr, in("rsi") len, in("rdx") advice, lateout("rax") r, out("rcx") _, out("r11") _, options(nostack)); }
    r
}
#[inline(never)]
fn sys_brk(addr: u64) -> i64 {
    let r: i64;
    unsafe { asm!("syscall", in("rax") 12u64, in("rdi") addr, lateout("rax") r, out("rcx") _, out("r11") _, options(nostack)); }
    r
}
#[inline(never)]
fn sys_mincore(addr: u64, len: u64, vec: u64) -> i64 {
    let r: i64;
    unsafe { asm!("syscall", in("rax") 27u64, in("rdi") addr, in("rsi") len, in("rdx") vec, lateout("rax") r, out("rcx") _, out("r11") _, options(nostack)); }
    r
}
#[inline(never)]
fn sys_shmget(key: i64, size: usize, flags: i64) -> i64 {
    let r: i64;
    unsafe { asm!("syscall", in("rax") 29u64, in("rdi") key, in("rsi") size, in("rdx") flags, lateout("rax") r, out("rcx") _, out("r11") _, options(nostack)); }
    r
}
#[inline(never)]
fn sys_shmat(shmid: i64, addr: u64, flags: i64) -> i64 {
    let r: i64;
    unsafe { asm!("syscall", in("rax") 30u64, in("rdi") shmid, in("rsi") addr, in("rdx") flags, lateout("rax") r, out("rcx") _, out("r11") _, options(nostack)); }
    r
}
#[inline(never)]
fn sys_shmctl(shmid: i64, cmd: i64, buf: u64) -> i64 {
    let r: i64;
    unsafe { asm!("syscall", in("rax") 31u64, in("rdi") shmid, in("rsi") cmd, in("rdx") buf, lateout("rax") r, out("rcx") _, out("r11") _, options(nostack)); }
    r
}
#[inline(never)]
fn sys_shmdt(addr: u64) -> i64 {
    let r: i64;
    unsafe { asm!("syscall", in("rax") 67u64, in("rdi") addr, lateout("rax") r, out("rcx") _, out("r11") _, options(nostack)); }
    r
}
