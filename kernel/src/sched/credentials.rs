...
fn with_security<R>(pid: Pid, f: impl FnOnce(&crate::process::SecurityContext) -> R) -> Option<R> {
    GLOBAL.lock().processes.get(&pid).map(|p| f(&p.security))
}

fn with_security_mut(pid: Pid, f: impl FnOnce(&mut crate::process::SecurityContext)) {
