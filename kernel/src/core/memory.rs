...
use crate::process_model::{BrkState, MmapState, Pid};

fn with_memory_mut(pid: Pid, f: impl FnOnce(&mut crate::process_model::MemoryContext)) {
    if let Some(p) = GLOBAL.lock().processes.get_mut(&pid) {
        f(&mut p.memory);
    }
}

pub fn set_tls_base(pid: Pid, addr: u64) {
    with_memory_mut(pid, |m| m.set_tls_base(addr));
}

pub fn set_current_tls_base(addr: u64) {
    set_tls_base(current_pid(), addr);
}
...
fn current_addr_space() -> Arc<crate::process_model::AddressSpace> {
    let pid = current_pid();
    let g = GLOBAL.lock();
    g.processes
        .get(&pid)
...
pub fn current_addr_space_opt() -> Option<Arc<crate::process_model::AddressSpace>> {
    let pid = current_pid();
    GLOBAL
        .lock()
        .processes
...
pub fn with_current_memory_mut<R>(
    f: impl FnOnce(&mut crate::process_model::MemoryContext) -> R,
) -> Option<R> {
    let pid = current_pid();
    GLOBAL
        .lock()
...
pub fn current_vmspace_id() -> u64 {
    ...
    g.processes
        .get(&pid)
        .and_then(|p| p.addr_space_root.map(|f| f.as_phys()))
        .unwrap_or(0)
}
...
pub struct MapsSnapshot {
    ...
    pub segments: alloc::vec::Vec<(
        u64,
        u64,
        frame::mm::vm::Perms,
        crate::process_model::MapSegLabel,
    )>,
}
...
pub fn process_maps(pid: Pid) -> Option<MapsSnapshot> {
    ...
    for v in &m.vmas {
        let label = match &v.backing {
            ...
            | crate::process_model::VmaBacking::Anonymous => MapVmaLabel::Anon,
        };
        vmas.push((
            v.start,
            v.end,
...
pub fn set_maps_layout(pid: Pid, layout: crate::process_model::MapsLayout) {
