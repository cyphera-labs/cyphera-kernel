...
fn build_user_stack_active(
    ...
    for s in envp.iter().rev() {
        p = p
            ...
            .ok_or(StackInitError::StackOverflow)?;
        write_user(p, s)?;
    ...
    if rsp >= strings_end || rsp < (stack_top - 16 * 4096) {
        return Err(StackInitError::StackOverflow);
    }
...
fn write_user(addr: u64, bytes: &[u8]) -> Result<(), StackInitError> {
    frame::user::copy_to_user(addr, bytes).map_err(|_| StackInitError::UserAccess)
}
