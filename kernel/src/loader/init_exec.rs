...
use crate::loader::stack_init::{AuxvInfo, build_user_stack};
...
pub fn exec_init() -> ! {
    ...
    let loaded = match crate::loader::elf::load_static(&buf, &mut vmspace) {
        Ok(l) => l,
    ...
    let pid = crate::process_model::register_with_vmspace(
        Some(vmspace),
        user_entry,
        rsp,
        loaded.image_end,
