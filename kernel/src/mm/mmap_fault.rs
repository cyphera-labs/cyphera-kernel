...
pub fn try_handle(cr2: u64, error: u64) -> bool {
    ...
    fault_in_page(page_addr, is_write, true)
}

pub fn populate_one_page(page_addr: u64) -> bool {
    let pre = sched::with_current_mmap(|m| {
        m.find_containing(page_addr)
            .map(|v| (v.prot, v.backing.clone()))
    });
    match pre {
        None => return false,
        Some((prot, backing)) => {
            ...
            if matches!(backing, VmaBacking::Shm { .. }) {
                return true;
            }
        }
    }
    if let Some(addr_space) = sched::current_addr_space_opt() {
        let mut vmspace = addr_space.vmspace.lock();
        if vmspace.translate(VirtAddr::new(page_addr)).is_some() {
            return true;
        }
    }
    fault_in_page(page_addr, false, false)
}

fn fault_in_page(page_addr: u64, is_write: bool, allow_stack_grow: bool) -> bool {
    ...
    let frame: PhysFrame<Size4KiB> = if is_shared_file {
        ...
        let frame: PhysFrame<Size4KiB> = match frame_alloc::alloc_frame() {
            ...
            None => {
                ...
                if allow_stack_grow {
                    sched::terminate_current_with_signal(crate::process_model::SIGKILL);
                }
                return false;
            }
        };
    ...
    match install {
        ...
        Err(frame::mm::vm::MapError::OutOfFrames) => {
            release();
            if allow_stack_grow {
                sched::terminate_current_with_signal(crate::process_model::SIGKILL);
            }
...
fn grow_stack(page_addr: u64) -> bool {
    ...
    match vmspace.map_one_frame(page, frame, Perms::USER_RW) {
        Ok(()) => {}
        Err(frame::mm::vm::MapError::OutOfFrames) => {
            frame_alloc::free_frame(frame);
            sched::sub_cgroup_charge(4096);
