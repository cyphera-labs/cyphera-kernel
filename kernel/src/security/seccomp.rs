...
use crate::security::bpf::BpfProgram;
...
pub fn install_filter(prog: Arc<BpfProgram>) {
    crate::core::seccomp_append_filter(prog);
}

pub fn install_filter_all_threads(prog: Arc<BpfProgram>) {
    crate::core::seccomp_append_filter_tgid(prog);
}

pub fn evaluate_for_syscall(tf: &frame::user::TrapFrame) -> u32 {
    let chain = match crate::core::current_seccomp_chain() {
        Some(c) => c,
    ...
    let data = SeccompData {
        nr: tf.syscall_nr() as i32,
        arch: AUDIT_ARCH_X86_64,
        instruction_pointer: tf.user_ip(),
        args: [
            tf.arg(0),
            tf.arg(1),
            tf.arg(2),
    ...
    let _ = chain;
    let chain = crate::core::current_seccomp_chain().unwrap_or_default();
